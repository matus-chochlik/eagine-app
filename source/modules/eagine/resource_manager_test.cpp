...
struct test_resource_manager_2 : eagitest::app_case {
    using launcher = eagitest::launcher<test_resource_manager_2>;

    test_resource_manager_2(auto& s, auto& ec)
      : eagitest::app_case{s, ec, 2, "2"}
    ...
    void clean_up() noexcept final {
        check(bool(res_1), "plain text is loaded");
        check(bool(res_2), "string list is loaded");
        bool content_is_ok{true};
        if(res_1 and res_2) {
            content_is_ok = res_1->starts_with(res_2->front()) and
                            res_1->ends_with(res_2->back());
        } else {
            content_is_ok = false;
        }
        check(content_is_ok, "content is ok");
...
auto test_main(eagine::test_ctx& ctx) -> int {
    ...
    test.once<test_resource_manager_2>();
