...
export module eagine.app.sky_viewer:resource;
...
//------------------------------------------------------------------------------
struct sky_viewer_resource_intf
  : abstract<sky_viewer_resource_intf>
  , sky_viewer_resource_signals {
...
class sky_viewer_resource_wrapper {
public:
    sky_viewer_resource_wrapper(unique_holder<Intf> impl) noexcept
      : _impl{std::move(impl)} {}
    ...
    auto signals() const noexcept -> sky_viewer_resource_signals& {
        return *_impl;
    }
...
protected:
    auto _settings_height(optional_reference<sky_viewer_resource_intf>) noexcept
      -> float;

    void _settings(
      const string_view head,
      optional_reference<sky_viewer_resource_intf>,
...
class sky_viewer_resources : public sky_viewer_resources_base {
public:
    explicit operator bool() const noexcept {
    ...
    auto load(
      ...
      video_context& video,
      Args&&... args) -> sky_viewer_resources& {
    ...
    auto use(video_context& video) noexcept -> sky_viewer_resources& {
        current().use(video);
        return *this;
    }

    auto load_if_needed(execution_context& ctx, video_context& video) noexcept
      -> sky_viewer_resources& {
        for(auto& resource : _loaded) {
