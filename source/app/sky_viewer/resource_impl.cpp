...
//------------------------------------------------------------------------------
void sky_viewer_resource_intf::signal_loaded() {
...
auto sky_viewer_resources_base::_settings_height(
  optional_reference<sky_viewer_resource_intf> impl) noexcept -> float {
    return 45.F +
           impl.member(&sky_viewer_resource_intf::settings_height).value_or(0.F);
}
//------------------------------------------------------------------------------
void sky_viewer_resources_base::_settings(
  const string_view head,
  optional_reference<sky_viewer_resource_intf> impl,
...
//------------------------------------------------------------------------------
auto sky_viewer_resources_base::_resource_name(const program_arg& arg)
  -> std::string {
...
//------------------------------------------------------------------------------
auto sky_viewer_resources_base::_resource_url(const program_arg& arg) -> url {
    if(url::is_url(arg.next())) {
        return url{arg.next()};
    }
...
//------------------------------------------------------------------------------
void sky_viewer_resources_base::update() noexcept {
...
//------------------------------------------------------------------------------
auto sky_viewer_resources_base::all_resource_count() noexcept -> span_size_t {
    return span_size(_names.size());
}
//------------------------------------------------------------------------------
void sky_viewer_resources_base::_add_name(std::string name) {
