...
//  Launchpad
//------------------------------------------------------------------------------
class sky_viewer_launchpad : public launchpad {
public:
...
//------------------------------------------------------------------------------
auto sky_viewer_launchpad::setup(main_ctx&, launch_options& opts) -> bool {
    opts.no_audio().require_input().require_video().with_stencil();
    return true;
}
//------------------------------------------------------------------------------
auto sky_viewer_launchpad::launch(execution_context& ec, const launch_options&)
  -> unique_holder<application> {
    return launch_with_video<sky_viewer>(ec);
}
//------------------------------------------------------------------------------
static auto sky_viewer_requirement_checker() noexcept {
...
//------------------------------------------------------------------------------
auto sky_viewer_launchpad::check_requirements(video_context& video) -> bool {
    return video.with_gl(sky_viewer_requirement_checker()).or_false();
}
//------------------------------------------------------------------------------
auto establish(main_ctx&) -> unique_holder<launchpad> {
    return {hold<sky_viewer_launchpad>};
}
//------------------------------------------------------------------------------
} // namespace eagine::app
