...
export module eagine.app.tiling_viewer:resource;
...
//------------------------------------------------------------------------------
struct tiling_viewer_resource_intf;
struct tiling_viewer_resource_signals {
    signal<void() noexcept> loaded;
    signal<void() noexcept> selected;
};
//------------------------------------------------------------------------------
struct tiling_viewer_resource_intf
  : abstract<tiling_viewer_resource_intf>
  , tiling_viewer_resource_signals {
...
class tiling_viewer_resource_wrapper {
public:
    tiling_viewer_resource_wrapper(unique_holder<Intf> impl) noexcept
      : _impl{std::move(impl)} {}
    ...
    auto signals() const noexcept -> tiling_viewer_resource_signals& {
        return *_impl;
    }
    ...
    auto load_if_needed(execution_context& ctx, video_context& video)
      -> tiling_viewer_resource_wrapper& {
        if(_impl) [[likely]] {
    ...
    auto use(video_context& video) -> tiling_viewer_resource_wrapper& {
        if(_impl) [[likely]] {
            _impl->use(video);
        }
        return *this;
...
//------------------------------------------------------------------------------
class tiling_viewer_resources_base {
public:
    signal<void() noexcept> loaded;
    signal<void() noexcept> selected;
...
protected:
    auto _settings_height(
      optional_reference<tiling_viewer_resource_intf>) noexcept -> float;

    void _settings(
      const string_view head,
      optional_reference<tiling_viewer_resource_intf>,
      const guiplus::imgui_api& gui) noexcept;

    auto _load_handler() noexcept {
        return make_callable_ref<&tiling_viewer_resources_base::_on_loaded>(
          this);
    }
    auto _select_handler() noexcept {
        return make_callable_ref<&tiling_viewer_resources_base::_on_selected>(
          this);
    }
...
class tiling_viewer_resources : public tiling_viewer_resources_base {
public:
    explicit operator bool() const noexcept {
    ...
    auto load(
      ...
      video_context& video,
      Args&&... args) -> tiling_viewer_resources& {
    ...
    auto use(video_context& video) noexcept -> tiling_viewer_resources& {
        current().use(video);
        return *this;
    }

    auto load_if_needed(execution_context& ctx, video_context& video) noexcept
      -> tiling_viewer_resources& {
        for(auto& resource : _loaded) {
